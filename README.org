...
- Adding/query tile (movies or series) with an
  [[Searching titles][expression language]];
...
For searching a title you can type  a key to search in any field or
you can use a simple expression language
([[https://en.wikipedia.org/wiki/Backus%E2%80%93Naur_form][BNF]] following):
